      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {topFilms.map((film, index) => {
            ...
            return (
              <Link key={index} to={`/film/${slug}`} className="block group h-full transition-all duration-300 hover:scale-[1.03]">
                    ...
                    {film.Festivals && film.Festivals.filter(f => f.Name_of_Festival?.trim()).length > 0 && (
                      <div className="mt-auto pt-4">
                        ...
                        <ul className="text-[0.7rem] space-y-0.5">
                          {film.Festivals.filter(f => f.Name_of_Festival?.trim()).slice(0, 3).map((festival, idx) => (
